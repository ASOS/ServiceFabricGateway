## Asos Octopus Deploy Service Fabric Application Deployment script
## 1. Changes made to ApplicationManifest path
## 2. Changes made to make the script idempotent
...
$ManifestFilePath = "$ApplicationPackagePath\pkg\ApplicationManifest.xml"
...
$AppVersionExists = (Get-ServiceFabricApplication | ? { $_.ApplicationTypeName -eq $AppTypeName -and $_.ApplicationName -eq $AppName -and $_.ApplicationTypeVersion -eq $AppTypeVersion }) -ne $null
  
# If an application which exactly matches the one being deployed is found, then do nothing (to make the script idempotent).
if($AppVersionExists)
{
    Write-Host "Currently deployed version matches existing active package version, nothing to deploy"
}
elseif ($IsUpgrade -and $AppExists)
{
    ...
    $parameters = @{
        ApplicationPackagePath =  "$ApplicationPackagePath\pkg"
    ...
    Publish-UpgradedServiceFabricApplication @parameters -UpgradeParameters $UpgradeParameters -CompressPackage -ErrorAction Stop
}
else
{
    ...
    Publish-NewServiceFabricApplication @parameters -CompressPackage -ErrorAction Stop
}